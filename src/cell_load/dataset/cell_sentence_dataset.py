...
class CellSentenceDataset(data.Dataset):
    """
    PyTorch Dataset for single-cell gene expression data, supporting multiple datasets and formats.

    Args:
        cfg: Configuration object.
        test (bool): If True, uses validation data.
        datasets (list, optional): List of dataset names/IDs.
        shape_dict (dict, optional): Mapping from dataset name to shape (num_cells, num_genes).
        adata (AnnData, optional): AnnData object for in-memory data.
        adata_name (str, optional): Name/ID for the AnnData object.
    """
...
class FilteredGenesCounts(CellSentenceDataset):
    """
    Dataset class that filters genes based on available protein/gene embeddings.

    Extends CellSentenceDataset to provide valid gene indices and embedding mappings for each dataset.
    """
...
class CellSentenceCollator(object):
    """
    Collate function for batching single-cell gene expression data with gene/protein embedding support.
    """
    ...
    def __init__(
        ...
        """
        Initialize the CellSentenceCollator.

        Args:
            cfg: Configuration object.
            valid_gene_mask (dict, optional): Mapping from dataset name to valid gene mask arrays.
            ds_emb_mapping_inference (dict, optional): Dataset-to-embedding index mapping.
            is_train (bool): Whether collator is for training or inference.
        """
        ...
        if self.cfg.loss.name == "tabular":
            if "batch_tabular_loss" in self.__dict__ and self.batch_tabular_loss:
                ...
                if n_shared > 0:
                    ...
                    if remainder > 0:
                        shared_genes = torch.cat(
                            [shared_genes, shared_indices[:remainder]]
                        )
                else:
    ...
    def sample_cell_sentences(
        ...
        if num_aug > 1:
            ...
            if down_umis > 0 and downsample_frac < 1.0:
                ...
                genes_sampled = torch.multinomial(
                    count_expr_dist.squeeze(), down_umis, replacement=True
                )
        ...
        for c, cell in enumerate(counts):
            ...
            cell_sentence_counts[c, :] = (
                100 * expression_weights[c, cell_sentences[c, :].to(torch.int32)]
            )
            ...
            task_gene_set = torch.tensor(
                task_sentence[c].tolist(), dtype=cell_sentences.dtype
            )
            ...
            if current_mask_count > target_mask_count:
                ...
                mask_indices = (
                    torch.where(potential_mask[1:])[0] + 1
                )  # +1 to adjust for offset
                ...
                if len(non_masked_indices) > 0 and additional_needed > 0:
                    additional_indices = non_masked_indices[
                        torch.randperm(len(non_masked_indices))[:additional_needed]
                    ]
